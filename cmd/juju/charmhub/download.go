...
--revision cannot be used together with --arch or --channel.
...
func (c *downloadCommand) Init(args []string) error {
	...
	harArch := c.arch != ArchAll && c.arch != ""
	if c.revision != -1 && (hasChannel || harArch) {
		return errors.New("--revision cannot be specified together with --arch or --channel")
	}
...
func (c *downloadCommand) Run(cmdContext *cmd.Context) error {
	...
	if c.revision == -1 {
		...
		cmdContext.Infof("Fetching %s %q revision %d using base %q",
			entityType, entity.Name, entity.Revision, normBase)
	}
